  ...
  return (
    ...
    >
          ...
          {searches.length === 0 ? (
            ...
            <View className="gap-2">
              {searches.map((entry) => (
                <View
                  key={entry.id}
                  className={`flex-row items-center justify-between rounded-xl px-3 py-2 ${cardInsetClass}`}
                >
                  <Pressable
      ...
      <View className="mb-3 flex-row gap-2">
        ...
        >
          <Text
            className={`text-xs font-black ${lang === "fr" ? "text-white" : "text-brand-700 dark:text-brand-100"}`}
          >
            FR
          </Text>
        </Pressable>
        ...
        >
          <Text
            className={`text-xs font-black ${lang === "en" ? "text-white" : "text-brand-700 dark:text-brand-100"}`}
          >
            EN
          </Text>
        </Pressable>
      ...
      {!isLoading && results.length === 0 ? (
        <StateText message="Lance une recherche pour afficher des livres." />
      ) : null}
