...
export const ListsScreen = () => {
  ...
  const onCreateList = async () => {
    ...
    try {
      ...
      showToast(
        err instanceof Error ? err.message : "Impossible de créer la liste",
        "error",
      );
    }
  };
  ...
  const onDeleteList = async (id: number) => {
    try {
      ...
      showToast(
        err instanceof Error ? err.message : "Impossible de supprimer la liste",
        "error",
      );
    }
  };
  ...
  return (
    <ScrollView className="flex-1" contentContainerClassName="pb-8">
      ...
      {!isLoading && lists.length === 0 ? (
        <StateText message="Aucune liste pour le moment." />
      ) : null}
              ...
              <View className="flex-row flex-wrap gap-3">
                {list.previewItems.map((entry) => (
                  <View key={entry.id} className="w-[22%]">
                    <CoverImage
                      uri={entry.book.thumbnail}
                      title={entry.book.title}
                      className="h-24 w-full rounded-lg"
                    />
                    <Text
                      className="mt-1 text-[10px] font-semibold text-slate-700 dark:text-slate-300"
                      numberOfLines={2}
                    >
                      {entry.book.title}
                    </Text>
