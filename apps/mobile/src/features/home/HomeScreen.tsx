    ...
    <ScrollView className="flex-1" contentContainerClassName="pb-8">
      ...
      <View className={`mb-4 rounded-2xl p-3 ${cardSurfaceClass}`}>
        <View className="mb-2 flex-row items-center justify-between">
          ...
          <Pressable
            className="rounded-full bg-slate-200 px-3 py-1.5 dark:bg-slate-800"
            onPress={() => void refresh()}
          >
            <Text className="text-xs font-bold text-brand-700 dark:text-brand-100">
              Actualiser
            </Text>
          </Pressable>
        </View>
      ...
      {!isLoading && !error ? (
        <>
          <ScrollView
            ...
            contentContainerClassName="mb-4 pr-2"
          >
          ...
          <View className={`mb-3 rounded-2xl p-3 ${cardSurfaceClass}`}>
            ...
            <Text className="text-xs text-slate-600 dark:text-slate-300">
              Sélection découverte pour varier vos lectures.
            </Text>
          </View>
          <ScrollView
            horizontal
          ...
          {exploreBooks.length === 0 ? (
            <Text className="text-sm text-slate-500 dark:text-slate-400">
              Aucun livre à explorer pour le moment.
            </Text>
          ) : null}
        </>
      ) : null}
